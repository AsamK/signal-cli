...
Specify positive integer display dimensions for the attachment at the same position in `--attachment`.
Use dimensions after applying video rotation. An empty string or omitted trailing entry keeps automatic image dimension detection; video dimensions are not detected.
Extra entries or malformed dimensions are rejected.
JSON-RPC uses `attachmentDimensions`, e.g. `["", "1080x1920"]`.
e.g.: `--attachment a.jpg clip.mp4 --attachment-dimensions '' 1080x1920`
