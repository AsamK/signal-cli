...
*dbus-send* [--system | --session] [--print-reply] --type=method_call --dest="org.asamk.Signal" /org/asamk/Signal[/<account-id>] org.asamk.Signal.<method> [string:<string argument>] [array:<type>:<array argument>]

Note: when daemon was started without explicit `-a ACCOUNT`, use the account's
object path returned by `org.asamk.SignalControl.listAccounts` or
`org.asamk.SignalControl.getAccount`. Numbered accounts use
`/org/asamk/Signal/_<phonenumber>`. Accounts without a phone number use
`/org/asamk/Signal/<ACI>`, with hyphens replaced by underscores.
...
For an account without a phone number, use its ACI (Account ID) with the
`signal-cli --dbus -a ACCOUNT` client. ACI selectors also work for numbered accounts.
An account without a phone number cannot infer the country code of a recipient;
use a complete international phone number, an ACI, or a Signal username.
...
listAccounts() -> accountList<ao>::
* accountList : Array of all attached accounts in DBus object path form

Exceptions: None

getAccount(identifier<s>) -> account<o>::
* identifier : Phone number or ACI of an attached account
* account : DBus object path of the account

Exceptions: org.asamk.SignalControl.Error.Failure if the account is unknown

startLink() -> deviceLinkUri<s>::
* deviceLinkUri : Provisioning URI to scan with the primary Signal device

finishLink(deviceLinkUri<s>, newDeviceName<s>) -> identifier<s>::
* deviceLinkUri : URI returned by startLink
* newDeviceName : Name for the linked device
* identifier : Phone number of the linked account, or its ACI when it has no phone number

Both linking methods support accounts with or without a phone number.

Exceptions: org.asamk.SignalControl.Error.Failure
...
- for multi-account mode: "/org/asamk/Signal/" + DBusIdentifier
* DBusIdentifier: account's phone number with underscore (_) replacing plus (+),
  or its ACI with underscores replacing hyphens when there is no phone number
...
* number : Your phone number, or an empty string for an account without a phone number

Exceptions: None

getSelfACI() -> aci<s>::
* aci : Your account's ACI (Account ID), available with or without a phone number
...
listRecipientIdentifiers() -> identifiers<as>::
* identifiers : String array of known recipient identifiers, including recipients without a phone number

Uses phone numbers where known and includes ACIs for recipients without a phone
number. Unlike `listNumbers`, this method also includes numberless recipients.
